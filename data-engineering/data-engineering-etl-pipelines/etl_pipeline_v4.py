...
with open('food_production_dataset.csv', 'r') as file:
    ...
    for row in reader:
        if int(row[1]) > 10:
            ...
            conn.execute('INSERT INTO produtos (produto, quantidade, preco_medio, receita_total, margem_lucro) VALUES (?, ?, ?, ?, ?)', (row[0], row[1], row[2], row[3], margem_lucro))
